...
| 2026-06-11 | DB Sage | ⚠️ **INCIDENTE (ambiente partilhado):** a `public.whatsapp_instances` foi esvaziada por actividade de terceiros no SIC GERAL (de 4 instâncias→1; tenant "lafemme"/restaurante e ~14 migrações de outros operadores aplicadas em 10-11 Jun). A instância da Porcelana desapareceu. **RECUPERADA** via re-inserção idempotente + religação do `instance_id` do agente (verificado: instance_id ligado=true). **Lição:** a 004 não está em `schema_migrations` (foi aplicada via Management API como dados, não migração rastreada) — dados soltos são voláteis no ambiente partilhado. Recomendação para @architect: instâncias/seeds críticos devem ser migrações rastreadas e re-aplicáveis. |
