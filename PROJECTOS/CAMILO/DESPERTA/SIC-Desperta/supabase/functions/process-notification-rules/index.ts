...
/** Retorna Date em Luanda (UTC-3) */
function toLuanda(d: Date): Date {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const now = new Date();
    // Hora atual em Luanda para comparar com trigger_time
    const brNow = toLuanda(now);
...
async function processDailyRule(supabase: any, rule: NotificationRule, now: Date): Promise<number> {
  ...
  // Range do dia em Luanda: meia-noite BRT = 03:00 UTC
  const brNow = toLuanda(now);
  ...
  console.log(`📅 Range Luanda (${brDateStr}): ${todayStartUTC} → ${todayEndUTC}`);
  ...
  if (tasks && tasks.length > 0) {
    ...
    for (const [responsavel, memberTasks] of sortedEntries) {
      ...
      const lines = memberTasks.map((t: any) => {
        const time = new Date(t.scheduled_at).toLocaleTimeString('pt-AO', {
          hour: '2-digit',
          minute: '2-digit',
  ...
  const brDateFormatted = brNow.toLocaleDateString('pt-AO', { timeZone: 'UTC' });
...
function formatMessage(template: string, task: Task, useDueDateTime: boolean): string {
  ...
  return template
    ...
    .replace(/\{\{data\}\}/g, scheduledDate.toLocaleDateString('pt-AO', { timeZone: 'Africa/Luanda' }))
    .replace(/\{\{hora\}\}/g, scheduledDate.toLocaleTimeString('pt-AO', {
      hour: '2-digit',
      minute: '2-digit',
      timeZone: 'Africa/Luanda'
    }))
...
async function checkAlreadySentToday(supabase: any, ruleId: string, now: Date): Promise<boolean> {
  // Checar se já enviou hoje em Luanda
  const brNow = toLuanda(now);
  const brDateStr = brNow.toISOString().split('T')[0];
  const todayStartUTC = `${brDateStr}T03:00:00.000Z`;
