...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const conversationText = messages && messages.length > 0
      ? messages
          .map((m: any) => {
            ...
            const date = new Date(m.created_at).toLocaleString("pt-AO");
    ...
    const activitiesText = activities && activities.length > 0
      ? activities
          .map((a: any) => {
            const date = new Date(a.created_at).toLocaleString("pt-AO");
    ...
    const transactionsText = transactions && transactions.length > 0
      ? transactions
          .map((t: any) => {
            const date = new Date(t.created_at).toLocaleString("pt-AO");
            const amountReais = convertTransactionAmount(t.amount, t.payment_method, t.payment_platform);
            return `[${date}] ${t.product_name}: Kz ${amountReais.toFixed(2)} - ${t.status}`;
          })
          .join("\n")
      : "Nenhuma transação";
    ...
    const dealsText = deals && deals.length > 0
      ? deals
          .map((d: any) => {
            const date = new Date(d.created_at).toLocaleString("pt-AO");
            const product = d.product?.name || "Produto não especificado";
            const stage = d.pipeline_stage?.name || d.status;
            return `[${date}] ${product}: Kz ${d.negotiated_price} - ${stage}${d.lost_reason ? ` (Perdido: ${d.lost_reason})` : ""}`;
          })
          .join("\n")
      : "Nenhum deal";
    ...
    const checkoutsText = checkouts && checkouts.length > 0
      ? checkouts
          .map((c: any) => {
            const date = new Date(c.created_at).toLocaleString("pt-AO");
            return `[${date}] ${c.product_name}: Kz ${c.amount} - ${c.status}${c.abandoned_at ? " (ABANDONADO)" : ""}`;
          })
          .join("\n")
      : "Nenhum checkout";
...
`.trim()
    ...
    const context = {
      ...
      available_products: products?.map((p: any) => `${p.name} (Kz ${p.price})`) || [],
