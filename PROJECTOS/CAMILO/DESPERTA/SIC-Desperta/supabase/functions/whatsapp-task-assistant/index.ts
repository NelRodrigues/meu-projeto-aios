    ...
    switch (queryType) {
      ...
      case "deals_open": {
        ...
        const list = deals?.map((d: any) =>
          `• ${d.lead?.name} - Kz ${Number(d.negotiated_price).toLocaleString("pt-AO")} (${d.pipeline_stage?.name})`
        ).join("\n") || "Nenhum";
      ...
      case "tasks_pending": {
        ...
        const list = tasks?.map((t: any) => {
          const date = t.due_datetime ? new Date(t.due_datetime).toLocaleDateString("pt-AO") : "Sem data";
      ...
      case "lead_info": {
        ...
        const dealInfo = deals?.map((d: any) =>
          `  • Kz ${Number(d.negotiated_price).toLocaleString("pt-AO")} - ${d.status} (${d.pipeline_stage?.name})`
        ).join("\n") || "  Nenhum";
      ...
      case "deal_info": {
        ...
        const previsao = fullDeal.expected_close_date
          ? new Date(fullDeal.expected_close_date).toLocaleDateString("pt-AO")
          : "Não definida";
        ...
        if (fullDeal.status === "won") {
          message = `Opa! O deal do *${lead?.name}* já foi fechado! 🎉\n\nFechamos por Kz ${valorNegociado.toLocaleString("pt-AO")}${desconto > 0 ? ` (demos ${desconto}% de desconto)` : ""} no dia ${new Date(fullDeal.won_at).toLocaleDateString("pt-AO")}. Produto: ${fullDeal.product?.name || "não especificado"}.`;
        } else if (fullDeal.status === "lost") {
          message = `Infelizmente perdemos o deal do *${lead?.name}* 😔\n\nMotivo: ${fullDeal.lost_reason || "não informado"}. Era um deal de Kz ${valorNegociado.toLocaleString("pt-AO")}.`;
        } else {
          ...
          message += `Tamo negociando Kz ${valorNegociado.toLocaleString("pt-AO")}`;
          if (desconto > 0) {
            message += ` (original era Kz ${valorOriginal.toLocaleString("pt-AO")}, ${desconto}% off)`;
          }
      ...
      case "team_tasks": {
        ...
        const list = tasks?.map((t: any) => {
          const time = t.due_datetime ? new Date(t.due_datetime).toLocaleTimeString("pt-AO", { hour: "2-digit", minute: "2-digit" }) : "";
    ...
    try {
      ...
      if (aiResponse.action === "create_task") {
        ...
        if (result.success) {
          ...
          resultData = result.data;
          responseMessage = aiResponse.message || `✅ Oportunidade criada: Kz ${Number(result.data.negotiated_price).toLocaleString("pt-AO")}`;
        } else {
          throw new Error(result.error);
        }
        ...
        responseMessage = aiResponse.message || `✅ Lead *${leadResult.data.name}* + Oportunidade Kz ${Number(dealResult.data.negotiated_price).toLocaleString("pt-AO")} criados!`;
