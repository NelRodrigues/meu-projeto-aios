...
async function handleBook(supabase: any, payload: BookPayload) {
  ...
  if (revenue !== undefined && revenue !== null) {
    ...
    const revenueText = revenue >= 1000000
      ? `Kz ${(revenue/1000000).toFixed(1).replace('.0', '')}M/mês`
      : `Kz ${(revenue/1000).toFixed(0)}k/mês`;
  ...
  updatePayload.context = `Lead preencheu formulário de agendamento no webinário. Empresa: ${company || 'N/I'}. Faturamento: Kz ${revenue ? (revenue/1000).toFixed(0) + 'k/mês' : 'N/I'}. ${slot_datetime ? 'Agendou call.' : 'Abaixo do threshold, aguardando contato.'}`;
  ...
  if (revenue >= 50000 && slot_datetime) {
    ...
    try {
      ...
      const calendarEvent = {
        ...
        description: `Lead: ${name}\nEmpresa: ${company || "N/A"}\nEmail: ${email || "N/A"}\nTelefone: ${phone}\nFaturamento: Kz ${(revenue || 0).toLocaleString("pt-AO")}\n\nAgendado via webinar pitch.`,
        start: {
