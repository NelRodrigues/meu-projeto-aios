...
// Multi-tenant: tenant_id passado no body da request pelo webhook/cron.
// Usado para filtrar queries e carregar API keys do tenant correcto.
let CURRENT_TENANT_ID: string | null = null;
...
/**
 * Verifica se está dentro do horário de trabalho (timezone Africa/Luanda)
 */
function isWithinWorkingHours(settings: AgentSettings): boolean {
  const now = new Date();
  // Converter para horário de São Paulo (UTC-3)
  const brTime = new Date(now.toLocaleString('en-US', { timeZone: 'Africa/Luanda' }));
...
async function notifyWhatsAppDisconnected(supabase: any, disconnectedInstance: WhatsAppInstance, errorMsg: string) {
  ...
  try {
    ...
    const data = brasilia.toLocaleDateString('pt-AO');
    const hora = brasilia.toLocaleTimeString('pt-AO', { hour: '2-digit', minute: '2-digit' });
...
async function getFullLeadContext(supabase: any, lead: Lead, settings: AgentSettings, instanceId?: string): Promise<string> {
  ...
  if (messages && messages.length > 0) {
    ...
    for (const msg of sortedMessages) {
      ...
      const time = new Date(msg.sent_at).toLocaleString('pt-AO', {
  ...
  if (deals && deals.length > 0) {
    ...
    for (const deal of deals) {
      ...
      const value = price ? `Kz ${Number(price).toLocaleString('pt-AO')}` : 'Valor não definido';
  ...
  if (allProducts && allProducts.length > 0) {
    ...
    for (const p of allProducts) {
      context += `- ${p.name}: Kz ${p.price?.toLocaleString('pt-AO') || '?'} - ${p.description?.substring(0, 80) || ''}\n`;
    }
  }
  ...
  if (tasks && tasks.length > 0) {
    ...
    for (const task of tasks) {
      const date = task.scheduled_at ? new Date(task.scheduled_at).toLocaleDateString('pt-AO') : 'Sem data';
  ...
  if (notes && notes.length > 0) {
    ...
    for (const note of notes) {
      const date = new Date(note.created_at).toLocaleDateString('pt-AO');
  ...
  try {
    ...
    if (calls && calls.length > 0) {
      ...
      for (const call of calls) {
        const date = new Date(call.started_at).toLocaleDateString('pt-AO');
  ...
  try {
    ...
    if (meetings && meetings.length > 0) {
      ...
      for (const meeting of meetings) {
        const date = new Date(meeting.started_at).toLocaleDateString('pt-AO');
  ...
  try {
      ...
      for (const conv of instaConvos) {
        ...
        if (instaMsgs && instaMsgs.length > 0) {
          ...
          for (const msg of sorted) {
            ...
            const time = new Date(msg.sent_at).toLocaleString('pt-AO', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
    ...
    if (engagement && engagement.engagement_score > 0) {
      ...
      if (engagement.last_interaction_at) {
        context += `- Última interação: ${new Date(engagement.last_interaction_at).toLocaleDateString('pt-AO')}\n`;
      }
    }
    ...
    if (lead.instagram_profile_id) {
      ...
      if (posts && posts.length > 0) {
        ...
        for (const post of posts) {
          const date = new Date(post.taken_at).toLocaleDateString('pt-AO');
      ...
      if (stories && stories.length > 0) {
        ...
        for (const story of stories) {
          const date = new Date(story.taken_at).toLocaleDateString('pt-AO');
...
async function buildAgentSystemPrompt(supabase: any, agent: AgentConfig, lead: Lead, settings: AgentSettings, materials?: any[]): Promise<string> {
  // Luanda timezone correto (toLocaleString garante dia correto independente do UTC)
  const nowUtc = new Date();
  const brTimeStr = nowUtc.toLocaleString('en-US', { timeZone: 'Africa/Luanda' });
...
`;
  ...
  const timeParts = nowUtc.toLocaleString('pt-AO', { timeZone: 'Africa/Luanda', hour: '2-digit', minute: '2-digit', hour12: false });
...
- DATA E HORA AGORA: ${dateStr} ${timeStr} (${dayOfWeek}) — Horário de Luanda
...
DATA DE HOJE: ${dateStr} (${dayOfWeek}) — Horário de Luanda (UTC-3)
  ...
  try {
    switch (tool.action_type) {
      ...
      case 'check_availability': {
        ...
        // Use Luanda timezone (UTC-3) for day boundaries
        ...
        // Converter atividades para intervalos ocupados em minutos desde meia-noite (horário Luanda)
        ...
        for (const a of activities) {
          ...
          if (a.end_datetime) {
            ...
            const endLuandaHour = (endDt.getUTCHours() - 3 + 24) % 24;
            const endLuandaMinute = endDt.getUTCMinutes();
            endMin = endLuandaHour * 60 + endLuandaMinute;
          }
      ...
      case 'schedule_meeting': {
        ...
        // Build scheduled_at with Luanda timezone (UTC-3)
          ...
          await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/process-notification-event`, {
            ...
            body: JSON.stringify({
              ...
              context: {
                ...
                meeting_data: meetDate ? meetDate.toLocaleDateString('pt-AO', { timeZone: 'Africa/Luanda' }) : '-',
                meeting_hora: meetDate ? meetDate.toLocaleTimeString('pt-AO', { timeZone: 'Africa/Luanda', hour: '2-digit', minute: '2-digit' }) : '-',
      ...
      case 'confirm_meeting': {
        ...
        try {
          ...
          const meetDateStr = meetDate ? meetDate.toLocaleDateString('pt-AO', { timeZone: 'Africa/Luanda' }) : '-';
          const meetHoraStr = meetDate ? meetDate.toLocaleTimeString('pt-AO', { timeZone: 'Africa/Luanda', hour: '2-digit', minute: '2-digit' }) : '-';
      ...
      case 'confirm_webinar': {
        ...
        const webinarDateStr = nextTuesday.toLocaleDateString('pt-AO', { timeZone: 'Africa/Luanda', weekday: 'long', day: 'numeric', month: 'long' });
...
async function processLeadMessage(
  ...
  // Multi-tenant: filtrar por CURRENT_TENANT_ID se disponível
  const stageName = lead.pipeline_stage_name || 'Novo';
  let agent: any = null;
  let messageInstanceId: string | null = null;
  const _tFilter = (q: any) => CURRENT_TENANT_ID ? q.eq('tenant_id', CURRENT_TENANT_ID) : q;

  // 3.0 Se agent_id veio na fila (trigger já roteou), usar direto
  if (queueItem.agent_id) {
    const { data: queueAgent } = await _tFilter(supabase
      .from('ai_sales_agents')
      .select('*')
  ...
  if (!agent) {
    ...
    if (messageInstanceId) {
      const { data: matchedAgent } = await _tFilter(supabase
        .from('ai_sales_agents')
        .select('*')
        .eq('is_active', true)
  ...
  if (!agent) {
    const { data: fallbackAgent } = await _tFilter(supabase
      .from('ai_sales_agents')
      .select('*')
      .eq('is_active', true)
      .eq('target_stages', '{}'))
...
async function processQueue(supabase: any): Promise<{ processed: number; errors: number }> {
  // Buscar configurações do agente ativo (filtrado por tenant se multi-tenant)
  let aq = supabase.from('ai_sales_agents').select('settings').eq('is_active', true);
  if (CURRENT_TENANT_ID) aq = aq.eq('tenant_id', CURRENT_TENANT_ID);
  const { data: agent } = await aq.limit(1).single();
  ...
  try {
    ...
    switch (step.action_type) {
      ...
      case 'ai_message': {
        ...
        const nowLuanda = new Date(Date.now() - 3 * 60 * 60 * 1000);
        const dataAtual = nowLuanda.toISOString().slice(0, 10);
        const horaAtual = nowLuanda.toISOString().slice(11, 16);

        let conversationHistory = `\n## DATA E HORA ATUAL: ${dataAtual} ${horaAtual} (horário de Luanda)\n`;
          ...
          if (hasActiveDeal || hasRecentCall || hasScheduledMeeting) {
            ...
            if (hasRecentCall) {
              for (const c of recentCalls!) {
                const callDate = new Date(c.started_at).toLocaleDateString('pt-AO');
            ...
            if (hasScheduledMeeting) {
              for (const m of scheduledMeetings!) {
                const meetDate = new Date(m.scheduled_at).toLocaleDateString('pt-AO');
...
async function processCadence(supabase: any): Promise<{ processed: number; errors: number }> {
  // Buscar settings globais para batch size (filtrado por tenant se multi-tenant)
  let gcq = supabase.from('ai_sales_agents').select('id, settings').eq('is_active', true);
  if (CURRENT_TENANT_ID) gcq = gcq.eq('tenant_id', CURRENT_TENANT_ID);
  const { data: globalAgent } = await gcq.limit(1).maybeSingle();
    ...
    try {
      ...
      // IA não deve mandar msg de prospecção na véspera de reunião marcada pelo vendedor
      {
        ...
        if (upcomingMeeting?.scheduled_at) {
          ...
          const meetDate = new Date(upcomingMeeting.scheduled_at).toLocaleDateString('pt-AO');
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Multi-tenant: extrair tenant_id do body (passado pelo webhook/cron)
    CURRENT_TENANT_ID = body.tenant_id || null;
    CURRENT_TENANT_SLUG = body.tenant_slug || null;

    // Se tenant_slug fornecido mas não tenant_id, resolver via tabela tenants
    if (CURRENT_TENANT_SLUG && !CURRENT_TENANT_ID) {
      const { data: t } = await supabase
        .from("tenants")
        .select("id")
        .eq("slug", CURRENT_TENANT_SLUG)
        .maybeSingle();
      if (t?.id) CURRENT_TENANT_ID = t.id;
    }

    // Hidratar API keys — tenta tenant-specific primeiro, fallback para global
    OPENAI_API_KEY = (await getIntegrationKey(supabase, "OPENAI_API_KEY", CURRENT_TENANT_SLUG || undefined)) || "";
    ANTHROPIC_API_KEY = (await getIntegrationKey(supabase, "ANTHROPIC_API_KEY", CURRENT_TENANT_SLUG || undefined)) || "";
    ...
    if (body.action === 'enqueue_message') {
      ...
      // Buscar debounce do agente (filtrado por tenant se multi-tenant)
      let agentQuery = supabase
        .from('ai_sales_agents')
        .select('settings')
        .eq('is_active', true);
      if (CURRENT_TENANT_ID) agentQuery = agentQuery.eq('tenant_id', CURRENT_TENANT_ID);
