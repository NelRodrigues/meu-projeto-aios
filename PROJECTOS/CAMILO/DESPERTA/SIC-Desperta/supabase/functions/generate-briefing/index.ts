...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const conversationText = messages && messages.length > 0
      ? messages
          ...
          .map((m: any) => {
            ...
            const date = new Date(m.created_at).toLocaleDateString("pt-AO");
    ...
    const activitiesText = activities && activities.length > 0
      ? activities
          .map((a: any) => {
            const date = new Date(a.created_at).toLocaleDateString("pt-AO");
    ...
    const transactionsText = transactions && transactions.length > 0
      ? transactions
          .map((t: any) => {
            const date = new Date(t.created_at).toLocaleDateString("pt-AO");
    ...
    const dealsText = deals && deals.length > 0
      ? deals
          .map((d: any) => {
            const date = new Date(d.created_at).toLocaleDateString("pt-AO");
            return `[${date}] ${d.product?.name || "Produto"}: Kz ${d.negotiated_price} - ${d.status}${d.lost_reason ? ` (${d.lost_reason})` : ""}`;
          })
          .join("\n")
      : "Sem deals";
