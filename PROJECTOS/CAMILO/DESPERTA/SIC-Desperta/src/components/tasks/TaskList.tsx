...
function formatDateTime(dateString: string | null) {
  ...
  // Forçar timezone de Luanda (Africa/Luanda)
  const options: Intl.DateTimeFormatOptions = { timeZone: 'Africa/Luanda' };
  const date = new Date(dateString);
  const now = new Date();
  
  // Converter para Luanda para comparação
  const dateLuanda = new Date(date.toLocaleString('en-US', options));
  const nowLuanda = new Date(now.toLocaleString('en-US', options));
  
  // Calcular diferença em dias (considerando apenas a data, não hora)
  const dateOnly = new Date(dateLuanda.getFullYear(), dateLuanda.getMonth(), dateLuanda.getDate());
  const nowOnly = new Date(nowLuanda.getFullYear(), nowLuanda.getMonth(), nowLuanda.getDate());
  const diffDays = Math.floor((dateOnly.getTime() - nowOnly.getTime()) / (1000 * 60 * 60 * 24));

  const timeStr = date.toLocaleTimeString('pt-AO', { hour: '2-digit', minute: '2-digit', timeZone: 'Africa/Luanda' });
  const dateStr = date.toLocaleDateString('pt-AO', { day: '2-digit', month: 'short', timeZone: 'Africa/Luanda' });
