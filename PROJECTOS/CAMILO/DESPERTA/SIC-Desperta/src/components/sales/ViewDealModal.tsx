  ...
  const handleRegisterCommitment = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Sinal de Kz ${amount.toLocaleString('pt-AO', { minimumFractionDigits: 2 })} registrado. Deal movido para Em Fechamento.`,
      });
          ...
          <ScrollArea className="max-h-[calc(90vh-120px)] pr-4">
            <div className="space-y-6">
                  ...
                  <Card>
                    <CardContent className="p-4">
                      ...
                      <p className="font-semibold">
                        {new Date(deal.expected_close_date).toLocaleDateString("pt-AO")}
                      </p>
                    </CardContent>
                  </Card>
                )}
                        ...
                        <div>
                          ...
                          {deal.commitment_date && (
                            <p className="text-xs text-emerald-600 dark:text-emerald-400">
                              Registrado em {new Date(deal.commitment_date).toLocaleDateString('pt-AO')}
                            </p>
                          )}
                        </div>
                      </div>
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground pt-4 border-t">
                <span>Criado em: {new Date(deal.created_at).toLocaleString("pt-AO")}</span>
                {deal.won_at && (
                  <span className="text-emerald-600 dark:text-emerald-400">
                    <Trophy className="h-3 w-3 inline mr-1" />
                    Ganho em: {new Date(deal.won_at).toLocaleString("pt-AO")}
                  </span>
                )}
                {deal.lost_at && (
                  <span className="text-red-600 dark:text-red-400">
                    <XCircle className="h-3 w-3 inline mr-1" />
                    Perdido em: {new Date(deal.lost_at).toLocaleString("pt-AO")}
                  </span>
                )}
              </div>
            </div>
