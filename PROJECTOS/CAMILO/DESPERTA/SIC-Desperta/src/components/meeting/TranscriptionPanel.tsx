    ...
    if (finishStatus === 'rescheduled') {
      ...
      if (activityId) {
        ...
        await supabase.from('company_activities').update({
          ...
          notes: `🔄 Reagendada para ${new Date(rescheduleDate).toLocaleDateString('pt-AO', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit', timeZone: 'Africa/Luanda' })}`,
        }).eq('id', activityId);
      ...
      toast({ title: '🔄 Reunião reagendada', description: `Nova data: ${new Date(rescheduleDate).toLocaleDateString('pt-AO', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit', timeZone: 'Africa/Luanda' })}` });
    ...
    if (finishStatus === 'no_show') {
      ...
      if (activityId) {
        supabase.from('company_activities').update({
          ...
          completed_at: new Date().toISOString(),
          notes: `❌ No-show em ${new Date().toLocaleDateString('pt-AO')}${noShowReason ? ` — Motivo: ${noShowReason}` : ''}`,
