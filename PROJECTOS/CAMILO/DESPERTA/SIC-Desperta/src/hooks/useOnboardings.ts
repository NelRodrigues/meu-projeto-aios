      ...
      if (tasks && tasks.length > 0) {
        ...
        await (supabase as any)
          ...
          .update({
            ...
            completed: false,
            notes: usersCreated
              ? `✅ Onboarding aprovado em ${new Date().toLocaleDateString('pt-AO')}. ${webhookResult?.summary?.users_created || 0} usuário(s) criado(s).`
              : `Onboarding aprovado em ${new Date().toLocaleDateString('pt-AO')}. Aguardando configuração.`,
            metadata: {
