          ...
          try {
            ...
            if (aiAnalysis) {
              ...
              if (negotiationDetails && !negotiationDetails.entrada_completa) {
                negFlags.push(`🟠 Entrada parcial — falta Kz${negotiationDetails.valor_faltante?.toFixed(2)}`);
              ...
              const briefingParts: string[] = [
                ...
                `**Valor:** Kz${(deal.negotiated_price || 0).toLocaleString('pt-AO', { minimumFractionDigits: 2 })}`,
                `**Data de fechamento:** ${new Date(wonAt || Date.now()).toLocaleDateString('pt-AO')}`,
                '',
              ];
