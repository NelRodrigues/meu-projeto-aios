  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      for (let i = 0; i < months; i++) {
        ...
        const monthName = monthDate.toLocaleDateString('pt-AO', { month: 'long', year: 'numeric' });
...
export const useSalesUrgentActions = (salesRepId?: string, limit: number = 10) => {
  return useQuery({
    ...
    queryFn: async () => {
      // Calcular início e fim do dia de HOJE em Luanda
      const now = new Date();
      const todayStart = new Date(now.toLocaleString('en-US', { timeZone: 'Africa/Luanda' }));
