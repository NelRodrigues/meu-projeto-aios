...
Passando para lembrar sobre o pagamento da {{parcela}} no valor de Kz {{valor}}, com vencimento em {{vencimento}}.
  ...
  return useMutation({
    mutationFn: async ({ leadId, message, parcela, valor, vencimento, produto }: SendBillingReminderParams) => {
      ...
      await supabase.from('company_activities' as any).insert({
        ...
        description: `Cobranca de Kz ${valor?.toLocaleString('pt-AO', { minimumFractionDigits: 2 }) || '?'} enviada via WhatsApp para ${lead.name}. Vencimento: ${vencimento || '-'}.`,
        status: 'completed',
        metadata: {
          parcela,
          valor,
