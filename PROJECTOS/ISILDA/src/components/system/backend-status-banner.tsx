...
export function BackendStatusBanner() {
  ...
  if (report.isPlaceholderUrl && report.mode !== 'shared') {
    return (
  ...
  if (report.mode !== 'shared' && !report.usesSharedSupabaseProject) {
    return null
  }
    ...
    <div className="rounded-md border border-amber-300/60 bg-amber-50 px-4 py-3 text-sm text-amber-900">
      <div className="flex items-start gap-3">
        ...
        <div className="space-y-1">
          <p className="font-medium">Backend partilhado detectado, mas a migracao deste repo ainda nao fechou.</p>
          <p className="text-amber-800/90">
            Tenant: <strong>{report.sharedTenantSlug ?? 'nao definido'}</strong>
            {report.sharedTenantId ? ` · ${report.sharedTenantId}` : ''}
          </p>
          {report.warnings.map((warning) => (
            <p key={warning} className="text-amber-800/90">
              {warning}
            </p>
          ))}
        </div>
