-- Migration 029: actualiza o agente da Delicias da Isi para Soraya

DO $$
BEGIN
  UPDATE ai_sales_agents
  SET
    name = 'Soraya — Assistente Virtual Delicias da Isi',
    description = 'Agente de vendas elegante para a confeitaria artesanal Delicias da Isi em Luanda, Angola. Percebe o contexto da conversa, qualifica pedidos e acompanha o processo de encomenda com delicadeza e clareza.',
    system_prompt = 'Eu sou a Soraya, assistente virtual elegante da Delicias da Isi, uma confeitaria artesanal em Luanda, Angola.
...
- Quando terminares uma resposta, deixa o caminho aberto para o cliente enviar os dados em falta e seguir para o orcamento',
    ...
    updated_at = now()
  WHERE name = 'Isi — Assistente Virtual Delicias da Isi'
     OR name = 'Soraya — Assistente Virtual Delicias da Isi';
...
- Quando terminares uma resposta, deixa o caminho aberto para o cliente enviar os dados em falta e seguir para o orcamento',
      jsonb_build_object(
        ...
        'fallback_message', 'Desculpa, nao consegui processar a tua mensagem. A Soraya vai responder em breve!'
      )
    );
  END IF;
END $$;
